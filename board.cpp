                        ...
                        //Что в последствии покажет ее местоположение
                            ...
                            for(int k(0); k < length; k++)
                            {
                                ...
                                for(int p(0); p < width; p++)
                                {
                                    ...
                                    if(temp_x >= mSize_of_board || temp_y >= mSize_of_board)
                                    {
                                        flag_of_error = 1;
                                    }
                                    //Если клетка не пренадлежит другой фигуре, то отправляем значение блока в клетку
                                    else if(mBoard[temp_x][temp_y].GetNumOfVal() == -1 || mBoard[temp_x][temp_y].GetVal() == mData[count_of_val])
                                    {
...
void Board::Input()
{
    ...
    while(temp_val == 0)
    {
        ...
        int temp_X,temp_Y;
        //Принимаем данные
        std::cout << "Введите значение клетки: ";
        get.GetIntMin(&temp_val, 1);
        std::cout << "Введите значение строки: ";
        get.GetInt(&temp_X, 1, mSize_of_board);
        temp_X--;
        std::cout << "Введите значение столбца: ";
        ...
        std::cout << "Значение успешно присвоенно!\n Для добавления нового значения введите \"0\"" << std::endl;
        get.GetInt(&temp_val);
    }
}
